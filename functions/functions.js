...
module.exports = {
  ...
  errorEmbed: function(message, channel, argument) {
    channel.send(new Discord.MessageEmbed().setDescription(`\\📛 **Erreur:** ${argument} \\📛`).setColor(config.color.red))
  },

  successEmbed: function(message, channel, argument) {
    channel.send(new Discord.MessageEmbed().setDescription(`\\✅ **Succès:** ${argument}`).setColor(config.color.green))
  },
